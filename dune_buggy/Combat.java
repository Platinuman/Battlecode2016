...
//
	public static RobotInfo chooseTarget(RobotInfo[] enemies) {
		...
		for (RobotInfo enemy : enemies) {
			if (enemy.attackPower > maxPower && (rc.canAttackLocation(enemy.location))) {
...
////			return false;
//		RobotInfo[] potentialAttackers = rc.senseHostileRobots(here, -1);
...
//		return true;
//	}
	public static void turretAttack(RobotInfo[] zombies, MapLocation closestDen) throws GameActionException {
		if(zombies.length > 0)
			rc.attackLocation(chooseTarget(zombies).location);
		else if(rc.canAttackLocation(closestDen))
			rc.attackLocation(closestDen);
	}
