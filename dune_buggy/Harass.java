...
//	}
	...
	public static void checkForDeadDen() throws GameActionException{
	if (targetLoc != null && rc.canSenseLocation(targetLoc)){
		RobotInfo bot = rc.senseRobotAtLocation(targetLoc); 
		if (bot == null || bot.type != RobotType.ZOMBIEDEN){
...
//			return;
		...
		if(targetLoc == null){
			...
			if(numDensToHunt > 1 && fate < 10 && !Util.isSurrounded(here) && rc.canSenseLocation(targetLoc) && rc.senseNearbyRobots(targetLoc, type.attackRadiusSquared, us).length > 15){
				rc.setIndicatorString(2, "moving on");
				MapLocation temp = targetDens[bestIndex];
				targetDens[bestIndex] = null;
				numDensToHunt--;
				bestIndex = Util.closestLocation(targetDens, here, targetDenSize);
				targetDens[bestIndex] = temp;
				if(bestIndex == -1) return;
				targetLoc = targetDens[bestIndex];
			}
		}
					...
					case DEN_NOTIF:
						...
						if(data[2] == 1){
							...
							if (!Util.containsMapLocation(targetDens, denLoc, targetDenSize)){
								...
								numDensToHunt++;
								if (!isGuard
										&& (!huntingDen || (targetLoc == null || here.distanceSquaredTo(denLoc) < here.distanceSquaredTo(targetLoc))) //test this
										&& type != RobotType.ARCHON) {
									targetLoc = denLoc;
									bestIndex = targetDenSize - 1;
									huntingDen = true;
							...
							if(deadDenIndex != -1){
								if(targetLoc != null && targetLoc.equals(denLoc)){
...
//						break;
					...
					if (closestIndex != -1 ){//&& type != RobotType.VIPER){
						...
						//killedDenSize++;
						if(targetLoc != null && targetLoc.equals(targetDens[closestIndex])){
							//rc.setIndicatorString(0, "here"); 
							targetLoc = null;
						}
...
//		RobotInfo[] enemies = Util.combineTwoRIArrays(enemyTurrets, turretSize, hostilesICanSee);
		...
		bytecodeIndicator += "gathering intel: " + (Clock.getBytecodeNum() - startB);
...
//		}
		...
		if(numEnemiesAttackingUs == 0){
			...
			checkForDeadDen();
			if(targetLoc == null || rc.getRoundNum() % 10 == 7)
				updateTargetLocWithoutSignals();
		}
		bytecodeIndicator += " Signals and targeting: " + (Clock.getBytecodeNum() - startB);
	...
	//	} 
		if (hostilesICanSee.length > 0) {
			startB = Clock.getBytecodeNum();
			doMicro(hostilesICanSee, enemiesICanShoot, friends, enemiesAttackingUs, numEnemiesAttackingUs);
			bytecodeIndicator += " Micro: " + (Clock.getBytecodeNum() - startB);
...
//				}
		...
		if(targetLoc != null)
			rc.setIndicatorString(2, "targetLoc = " + targetLoc.toString());
		else rc.setIndicatorString(2, "targetLoc is null");
